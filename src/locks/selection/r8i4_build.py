...
settled-length gate, unchanged).  Per (block, kv-head), for a finalized page:
    ...
    V    = dc^T U8 / S8        (d, 8)     int4, per-column bf16 scales
                               (scale = absmax/7), packed COLUMN-major
                               (8 x d/2 bytes, lo nibble = even d row)
...
deviations from ``build.py``:
  ...
  * V is quantized ``round(V/(absmax/7)).clamp(-8, 7)`` (the standalone's
    asymmetric int4 range), not the symmetric +-7 of ``build._quant``.
...
from .r8i4_state import RNK, R8i4State
...
def _eigh_chunked(Gf: torch.Tensor):
    ...
    if _R8I4_EIGH == "syevd":
        ...
        return eigh_syevd(Gf, _EIGH_CHUNK)
    n = Gf.shape[-1]
...
def _write_post(st: R8i4State, mu, dc, S2, U, tag_new, lidx, bidx) -> None:
    ...
    # int4 basis, per-column scales (absmax/7; asymmetric clamp to -8..7)
    vsc = (V.abs().amax(2) / 7.0).clamp_min(1e-8)          # (N, n_kv, 8)
    Vq = torch.round(V / vsc[:, :, None]).clamp(-8, 7)
    # pack column-major: (N, n_kv, 8, d/2) two int4 per byte (lo = even row)
    Vc = Vq.permute(0, 1, 3, 2).contiguous().to(torch.int8)  # (N, n_kv, 8, d)
    lo = (Vc[..., 0::2] & 0xF).to(torch.uint8)
    hi = (Vc[..., 1::2] & 0xF).to(torch.uint8)
    v4 = (lo | (hi << 4)).contiguous()
